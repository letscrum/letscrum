...
func CreateProject(name string, displayName string, createdUserId int64) error {
	p := Project{
		Name:        name,
...
func UpdateProject(name string, displayName string) error {
	p := Project{
		DisplayName: displayName,
	}
	if err := models.DB.Model(&Project{}).Where("name = ?", name).Update("display_name", p.DisplayName).Error; err != nil {
		return err
	}
	return nil
}
...
func DeleteProject(name string) error {
	if err := models.DB.Where("name = ?", name).Delete(&Project{}).Error; err != nil {
		return err
	}
	return nil
}
